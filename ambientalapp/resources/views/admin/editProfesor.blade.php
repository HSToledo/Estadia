        ...
        <div class="row">
          <div class="col-12">
            <div class="card card-primary">
              ...
              <!-- /.card-header -->
              <!-- form start -->
              <form method="post" action="{{route('admin.updateProfesor',$profesor->id)}}" id="profesorEditForm">
                @csrf
                <div class="card-body">
                  ...
                  <div class="input-group mb-3 form-group">
                    ...
                    <div class="custom-control custom-radio" style="margin-right: 10%">
                      <input class="custom-control-input" type="radio" id="hombre" name="sexo" value="masculino" @if ($profesor->sexo == 'masculino') checked='checked' @endif>
                      <label for="hombre" class="custom-control-label">Masculino</label>
                    </div>
                
                    <div class="custom-control custom-radio">
                      <input class="custom-control-input" type="radio" id="mujer" name="sexo" value="femenino"  @if ($profesor->sexo == 'femenino') checked='checked' @endif>
                  ...
                  <div class="form-group">
                    ...
                    <select class="form-control select2" style="width: 100%; height:15%;" name="municipio" id="municipio">
                      @foreach ($municipios as $municipio)
                        <option value="{{$municipio->nombre}}" @if ($profesor->municipio == $municipio->nombre) selected='selected' @endif >{{$municipio->nombre}}</option>
                      @endforeach
                    </select>
                  </div>
                  <div class="form-group">
                    ...
                    <select class="form-control select2" style="width: 100%; height:15%;" name="escuela" id="escuela">
                      @foreach ($escuelas as $escuela)
                        <option value="{{$escuela->id}}" @if ($profesor->idEscuela== $escuela->id) selected='selected' @endif >{{$escuela->nombre}}</option>
                      @endforeach
                    </select>
                  </div>
                  <div class="form-group">
              ...
              <div class="card-footer">
                <a href="{{route('admin.indexProfesores')}}"><button class="btn btn-danger">Cancelar</button></a>
              </div>
            </div>
            <!-- /.card -->
          </div>
